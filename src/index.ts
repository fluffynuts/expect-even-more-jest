    ...
    expect.extend({
        ...
        toBeFile(actual: string) {
            return runAssertions(this, () => {
                ...
                assert(!!actual, msg);
                assert(fileExistsSync(actual), msg);
                return msg;
            });
        },
        toBeFolder(actual: string) {
            return runAssertions(this, () => {
                ...
                assert(!!actual, msg);
                assert(folderExistsSync(actual), msg);
                return msg;
            });
        },
        toHaveContents(actual: string, expected: string | Buffer) {
            return runAssertions(this, () => {
                assert(!!actual, () => `file path not provided`);
                assert(fileExistsSync(actual), () => `file not found: ${actual}`);
                const contents = tryReadContentsOf(actual);
                const msg = () => `expected '${actual}' to contain:\n${expected}\nbut found:\n${contents}`;
                assert(contents !== undefined, () => `unable to read file at: ${actual}`);
                const toExpect = typeof expected === "string"
                    ? Buffer.from(expected)
                    : expected;
                assert((toExpect as any) instanceof Buffer, () => `expected contents must be a string or a Buffer`);
                assert(areEquivalent(contents!, toExpect), msg);
                return msg;
            });
        },
...
function tryReadContentsOf(file: string): Buffer | undefined {
    try {
...
function areEquivalent(
    ...
    for (let idx = 0; idx < buffer1.length; idx++) {
